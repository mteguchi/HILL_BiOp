...
MCMC.n.chains <- 5
MCMC.n.samples <- 750000
MCMC.n.burnin <- 650000
MCMC.n.thin <- 50
...
#load.module('dic')
jags.params <- c('theta.1', "theta.2", 'sigma.pro1', "r",
                 'mu', 'y', 'X', 'deviance', 'loglik')

jm <- jags(jags.data,
           inits = NULL,
           parameters.to.save= jags.params,
           model.file = 'models/model_SSAR1_norm_negbin_theta.txt',
...
if (save.fig)
  ggsave(plot = p.1,
         filename = 'figures/predicted_counts_JM_norm_negbin_theta_1999.png',
         dpi = 600)

if (save.RData)
  saveRDS(results,
          file = paste0('RData/SSAR1_norm_negbin_theta_JM_1999_', Sys.Date(), '.rds'))
