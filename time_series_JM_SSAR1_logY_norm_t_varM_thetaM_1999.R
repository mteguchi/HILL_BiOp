...
if (save.fig)
  ggsave(plot = p.1,
         filename = 'figures/predicted_counts_JM_logY_norm_t_varM_thetaM_1999.png',
         dpi = 600)

if (save.RData)
  saveRDS(results,
          file = paste0('RData/SSAR1_logY_norm_t_varM_thetaM_JM_1999_', Sys.Date(), '.rds'))
...
if (plot.fig){
  ...
  mcmc_trace(jm$samples, c("mu", "sigma.obs",
                           "df"))
  mcmc_dens(jm$samples, c("mu", "sigma.obs",
                          "df"))
