...
#load.module('dic')
jags.params <- c('theta.1', 'sigma.pro1', "r",
                 'mu', 'y', 'X', 'deviance', 'loglik')

jm <- jags(jags.data,
           inits = NULL,
           parameters.to.save= jags.params,
           model.file = 'models/model_SSAR1_lnorm_negbin.txt',
...
if (save.fig)
  ggsave(plot = p.1,
         filename = 'figures/predicted_counts_JM_norm_negbin_1999.png',
         dpi = 600)

if (save.RData)
  saveRDS(results,
          file = paste0('RData/SSAR1_norm_negbin_JM_1999_', Sys.Date(), '.rds'))
...
if (plot.fig){
  ...
  mcmc_trace(jm$samples, c('theta.1', "mu", "r",
                           "sigma.pro1"))
  mcmc_dens(jm$samples, c('theta.1', "mu",
                          "sigma.pro1", "r"))
