...
jm <- jags.model(file = 'models/model_SSAR1_month_var.txt',
                 data = bugs.data,
...
results.JM_SSAR1_month_var <- list(data.1 = data.1.JM,
                               bugs.data = bugs.data,
                               summary.zm = summary.zm,
                               Xs.stats = Xs.stats,
                               Xs.year = Xs.year,
...
if (save.fig)
  ggsave(plot = p.1,
         filename = 'figures/predicted_counts_JM_month_var_1999.png',
         dpi = 600)

if (save.RData)
  save(results.JM_SSAR1_month_var,
       file = paste0('RData/SSAR1_month_var_JM_1999_', Sys.Date(), '.RData'))
...
if (plot.fig){
  ...
  mcmc_trace(zm, c('theta', "sigma.pro1", "sigma.pro2", "sigma.obs"))
  mcmc_dens(zm, c('theta', "sigma.pro1", "sigma.pro2", "sigma.obs"))
