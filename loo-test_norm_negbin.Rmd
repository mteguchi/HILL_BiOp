...
Create data:
```{r}
jm.out <- readRDS('RData/SSAR1_norm_negbin_var_theta_JM_1999_jagsUI_2018-10-01.rds')

data.1 <- jm.out$data.1
means <- jm.out$jm$mean
...
```{r}
if (!file.exists("RData/RData/SSAR1_norm_negbin_var_theta_loo_test.rds")){
  MCMC.n.chains <- 5
  ...
  saveRDS(results,
          file = paste0('RData/SSAR1_norm_negbin_var_theta_loo_test.rds'))
} else {
  results <- readRDS(file = "RData/SSAR1_norm_negbin_var_theta_loo_test.rds")
}
