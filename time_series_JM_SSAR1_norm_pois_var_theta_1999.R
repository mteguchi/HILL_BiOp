...
jm <- jags(jags.data,
                   ...
                   model.file = 'models/model_SSAR1_norm_pois_var_theta.txt',
...
if (save.fig)
  ggsave(plot = p.1,
         filename = 'figures/predicted_counts_JM_norm_pois_var_theta_1999.png',
         dpi = 600)

if (save.RData)
  saveRDS(results,
       file = paste0('RData/SSAR1_norm_pois_var_theta_JM_1999_', Sys.Date(), '.rds'))
...
if (plot.fig){
  ...
  idx <- c(1:length(jags.data$y))[is.na(jags.data$y)]
  y.fig <- apply(as.matrix(idx), MARGIN = 1, FUN = function(x) paste0("y[", x, "]"))
  mcmc_trace(jm$samples, y.fig[1:12])
  mcmc_trace(jm$samples, y.fig[13:24])
  mcmc_trace(jm$samples, y.fig[25:36])
  mcmc_trace(jm$samples, y.fig[37:48])
  mcmc_trace(jm$samples, y.fig[49:60])
}
