...
p.JM.all <- ggplot(data.1.JM) + 
  geom_point(aes(x = Frac.Year, 
                 y = Nests, size = num_days)) + 
...
So, for this part of the analysis, i.e., filling in the recent missing data, I use data from 2011 to the most recent.

```{r}
data.1.JM.2011 <- filter(data.1.JM, Year > 2010)
p.JM.2011 <- ggplot(data.1.JM.2011) + 
  geom_point(aes(x = Frac.Year, 
                 y = Nests)) + 
  geom_line(aes(x = Frac.Year, y = Nests)) +
...
p.JM.2011
if (save.fig) ggsave(filename = 'figures/JM_2011.png',
                     plot = p.JM.all,
                     dpi = 600, height = 6, width = 8, units = "in")
...
```{r warning=FALSE}
p.JM.monthly <- ggplot(data.1.JM.2011) + 
  geom_point(aes(x = Month, y = Nests, color = f.year)) + 
  geom_line(aes(x = Month, y = Nests, color = f.year)) +
