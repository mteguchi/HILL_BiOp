...
#load.module('dic')
jags.params <- c('theta.1', "theta.1",  "r",
                 'mu', 'y', 'X', 'deviance', 'loglik')

jm <- jags(jags.data,
           inits = NULL,
           parameters.to.save= jags.params,
           model.file = 'models/model_SSAR1_pois_negbin_theta.txt',
...
if (save.fig)
  ggsave(plot = p.1,
         filename = 'figures/predicted_counts_JM_pois_negbin_theta_1999_jagsUI.png',
         dpi = 600)

if (save.RData)
  saveRDS(results,
          file = paste0('RData/SSAR1_pois_negbin_theta_JM_1999_jagsUI_', Sys.Date(), '.rds'))
...
if (plot.fig){
  ...
  mcmc_dens(jm$samples, c('theta.1', "theta.2", "mu","r"))
