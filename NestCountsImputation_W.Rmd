...
# first log likelihood value is NA... why's that? but I'll worry about that later.
pareto.df <- data.frame(y = data.y[2:length(data.y)],
                        khat = pareto.k,
                        datapoint = seq(from = 1, to = (length(data.y) - 1)))
...
The worst offender seems to be an extreme point... 
...
```{r model2, cache=TRUE, include=TRUE}
M <- readRDS(paste0("RData/jagsout_SSAR1_logY_norm_norm_varM_W_2006_2014_", run.date, ".rds"))
...
M <- readRDS(paste0("RData/jagsout_SSAR1_logY_norm_t_varM_W_2006_2014_", run.date, ".rds"))
...
#Model comparison
Estimated Pareto k statistics indicated that models 4, 7, 13, 16, and 17 had better predictive performance than the rest. We compared these models using using ELPD values.

```{r ModelComparison, include=TRUE}
M.compare <- compare(M4.loo.out, M7.loo.out, M13.loo.out, M16.loo.out,  M17.loo.out)
print(M.compare)
```

Model 17, which contained month-specific slopes and two variances, turned out to be the best.  
...
```{r posterior_M17, fig.cap="Figure. Posterior distribuitons of slope parameters for M17"}
...
```{r posterior_M17_2, fig.cap="Figure. Posterior distributions for the standard deviation of the process (sigma.pro1), standard deviation of observations (sigma.obs), and degrees of freedom (df) parameter."}
bayesplot::mcmc_dens(M17.jm$samples, c("sigma.pro1", "sigma.pro2", "sigma.obs", "df"))
```
...
```{r}
M17 <- readRDS(paste0("RData/jagsout_SSAR1_logY_norm_t_var_thetaM_W_2006_2014_", run.date, ".rds"))
...
```{r}
M17 <- readRDS(paste0("RData/jagsout_SSAR1_logY_norm_t_var_thetaM_W_2006_2017_", run.date1, ".rds"))
...
write.csv(Xs.season, file = "data/estimatedX_M17.csv",
          quote = F, row.names = F)
